...
Процедура РассчитатьНачисления()
	...
	Пока Результат.Следующий() Цикл
		...
		ДвижениеУдержание.Подразделение = Движение.Подразделение;
		ДвижениеУдержание.Сотрудник = Движение.Сотрудник;
		ДвижениеУдержание.ПериодРегистрации = Дата;
		ДвижениеУдержание.Начисление = Перечисления.ВКМ_СпособРасчета.Оклад;
		ДвижениеУдержание.БазовыйПериодНачало = Движение.ПериодДействияНачало;
		ДвижениеУдержание.БазовыйПериодКонец = Движение.ПериодДействияКонец;
...
Процедура РассчитатьНачисленияОтпусков()
	...
	Если Результат.Количество() > 0 Тогда
		Пока Результат.Следующий() Цикл
			Движение = Движения.ВКМ_ОсновоныеНачисления[Результат.НомерСтроки - 1];
			Движение.Сумма = (Результат.Сумма / Результат.РабочихДней) * Результат.ДнейОтпуска;
			...
			ДвижениеУдержание.Сотрудник = Движение.Сотрудник;
			ДвижениеУдержание.ПериодРегистрации = Дата;
			ДвижениеУдержание.Начисление = Перечисления.ВКМ_СпособРасчета.Отпуск;
...
Процедура РассчитатьУдержания()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ВКМ_УдержанияБазаВКМ_ОсновоныеНачисления.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновоныеНачисления.СуммаБаза, 0) КАК СуммаБаза
	|ИЗ
	|	РегистрРасчета.ВКМ_Удержания.БазаВКМ_ОсновоныеНачисления(&Измерения, &Измерения, &Разрезы,
	|		Регистратор = &Регистратор) КАК ВКМ_УдержанияБазаВКМ_ОсновоныеНачисления
	|ГДЕ
	|	ВКМ_УдержанияБазаВКМ_ОсновоныеНачисления.РегистраторРазрез = &Регистратор";
	Измерения = Новый Массив;
	Измерения.Добавить("Подразделение");
	Измерения.Добавить("Сотрудник");
